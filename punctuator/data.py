...
def load(path):
    ...
    with open(path, 'r') as f:
        for l in f:
            d.append(eval(l)) # pylint: disable=eval-used
    return d
...
def build_vocabulary(word_counts):
    return [wc[0]
            ...
            if wc[1] >= MIN_WORD_COUNT_IN_VOCAB and wc[0] != UNK][:MAX_WORD_VOCABULARY_SIZE] # Unk will be appended to end
...
def write_processed_dataset(input_files, output_file):
    """
    data will consist of two sets of aligned subsequences (words and punctuations) of MAX_SEQUENCE_LEN tokens
        ...
        with open(input_file, 'r', encoding='utf-8') as text:
            ...
            for line in text:
                ...
                for token in line.split():
                    ...
                    token = PUNCTUATION_MAPPING.get(token, token)
                    ...
                    if token in CRAP_TOKENS:
                        continue

                    if token.startswith(PAUSE_PREFIX):
                    ...
                    if len(current_words) == MAX_SEQUENCE_LEN: # this also means, that last token was a word
                        ...
                        assert len(current_words
                                   ) == len(current_punctuations) + 1, "#words: %d; #punctuations: %d" % (len(current_words), len(current_punctuations))
                        assert current_pauses == [] or len(current_words) == len(current_pauses
                                                                                 ), "#words: %d; #pauses: %d" % (len(current_words), len(current_pauses))
                        ...
                        if last_eos_idx == 0:
                            ...
                            subsequence = [current_words[:-1] + [word_vocabulary[END]], current_punctuations, current_pauses[1:]]
